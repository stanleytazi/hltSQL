...
bool indexInsert(struct index *idx,const char *t,unsigned int d,float tf)
{
...
struct result* indexFind(struct index *idx,const char *t,unsigned int *num)
{
	struct posting *p;
	char *invertedlist;
	...
	if(*num != 0)
	{
		...
		if(p)
		{
			...
			if(!vector_get_vbyte(p->invertedlist,&curdoc))
				goto error;
			result[i].d = curdoc;
			if(!vector_get_float(p->invertedlist,&tf))
				goto error;
			result[i].w = tf * log(idx->fileMap->total/(*num));
			i++;
			for(j=1; j<p->count; j++)
...
int create_test()
{
	...
	while(fscanf(f,"%s %d %lf\n",c,&tmp,&w) != EOF)
	{
		...
		indexInsert(idx,c,d,w); /* add data to index */
		i++;
		if(i == 100)
			i = 0;
	}
...
int invertedlist_bigger_than_page_test()
{
	...
	for(i=0; i<3000; i++)
	{
		if(!indexInsert(idx,"test",d++,1))
				return -1;
	}
	if(!indexDump(idx))	/* index dump */
		return -1;
	r = indexFind(idx,"test",&num);
	if(r)
	{
		for(i=0; i<num; i++)
		{
...
int load_test()
{
	...
	result = indexFind(idx,"ddm",&num);
	if(result)
	{
		for(i=0; i<num; i++)
		{
			printf("<%d %f>",result[i].d,result[i].w);
		}
		free(result);
	}
	getchar();
